...
//
//  Created by iujie on 23/11/2022.
//

import Foundation
import OpenTok

class ImageCapturer: NSObject, OTVideoCapture {
    ...
    fileprivate let captureImage: UIImage
    ...
    fileprivate func resizeAndPad(image img: UIImage) -> CGImage {
        ...
        ctx?.scaleBy(x: 1, y: -0.9)
        ctx?.translateBy(x: 0, y: -destSizes.rect.size.height + 90)
    ...
    func initCapture() {
        timer.setEventHandler {
            let padded = self.resizeAndPad(image: self.captureImage)
            self.consume(frame: padded)
        }
        timer.schedule(deadline: DispatchTime.now(), repeating: DispatchTimeInterval.milliseconds(1000))
    }
